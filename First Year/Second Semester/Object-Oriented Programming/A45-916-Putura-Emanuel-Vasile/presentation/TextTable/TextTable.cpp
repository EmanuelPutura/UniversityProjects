...
int TextTable::addRow(size_t argc, ...) {
    ...
    for (size_t i = 0; i < argc; ++i) {
        ...
        if (((int)columns_size[i] - (int)strlen(data)) < MIN_PADDING * 2) resize(i, strlen(data) + MIN_PADDING * 2);
    ...
    if (build_up) rows.push_back(buildRowDelimitator("-"));
    rows.push_back(row_data);
    rows.push_back(buildRowDelimitator("-"));
