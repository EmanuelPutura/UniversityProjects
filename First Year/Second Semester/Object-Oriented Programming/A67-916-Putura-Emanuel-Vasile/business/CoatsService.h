...
private:
    ...
    int payment = 0;
public:
    CoatsService(AbstractRepository<TrenchCoat>& repository, AbstractLaunchRepository<TrenchCoat>& shopping_basket) : repository(repository), shopping_basket(shopping_basket),
    ...
    inline void showShoppingBasket() const {
        Launcher().launch(shopping_basket.getPath());
    };
