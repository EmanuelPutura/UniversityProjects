...
void removeCharTextTable(char* string, const char character) {
	int count = 0;
	if (string[0] != character) string[count++] = string[0];
	for (int i = 1; string[i]; i++)
		if (string[i] != character || (string[i - 1] != character && string[i + 1] != character))
...
int setCellDataTextTable(TextTable* text_table, int row, int column, char* value, int span) {
	if (row < 0 || column < 0 || row >= text_table->rows || column >= text_table->columns) return -1;
	char** data = (char**)getDataDA(text_table->dArray);
...
int addRowTextTable(TextTable* text_table, bool header, int argc, ...) {
	...
	for (int i = 0; i < text_table->columns; ++i) {
		...
		if ((size_t)text_table->column_size[i] < (strlen(column_data) + 2))
			resizeTextTable(text_table, true, i, strlen(column_data) + CELL_PADDING);

		for (size_t j = 0; j < (text_table->column_size[i] - strlen(column_data)) / 2; ++j) {
...
char* getStringReprTextTable(TextTable* text_table) {
	char** data = (char**)getDataDA(text_table->dArray);
	char* return_data = (char*)malloc(getWidthTextTable(text_table) * (text_table->rows * 2 + 1) * sizeof(char) + text_table->rows * 2 + 2);
	if (return_data == NULL) return NULL;
	return_data[0] = 0;

	int sum = 0;
	for (int i = 0; i < text_table->rows * 2 + 1; ++i) {
		strcat(return_data, data[i]);
		strcat(return_data, "\n");
	}
